...
+ (void)showErrorWithStatus:(NSString *)string withViewController:(UIViewController *)viewcontroller
{
    if ([string isEqualToString:NetworkLost])
    {
        [AZNotification showNotificationWithTitle:string controller:viewcontroller notificationType:AZNotificationTypeWarning];
    }
    else
    {
        [self sharedView];
...
+ (void)showImage:(UIImage *)image status:(NSString *)string withColor : (UIColor *) color ; {
    [[self sharedView].hudView setBackgroundColor:[UIColor whiteColor]];
    //SVProgressHUDForegroundColor = [UIColor whiteColor];
    [[[self sharedView] stringLabel]setTextColor:SVProgressHUDForegroundColor];
    [self showImage:image status:string maskType:SVProgressHUDDefaultMaskType];
}
...
//        SVProgressHUDForegroundColor = [UIColor blackColor];
        ...
        UIImage* errorImage = [UIImage imageNamed:@"saddy"];
